...
class WhisperKitTranscriptionService: ObservableObject {
    ...
    /// 詳細ログ出力設定（問題診断のため有効化）
    private let verboseLoggingEnabled: Bool = true
    ...
    private init() {
        ...
        if !bundledModels.isEmpty {
            ...
            print("📦 Bundled models registered as downloaded: \(bundledModels)")
        }
    ...
    private func loadDownloadedModelsState() {
        ...
        if let savedModels = defaults.array(forKey: "downloadedWhisperModels") as? [String] {
            ...
            print("📱 Loaded downloaded models state: \(downloadedModels)")
        }
    }
    ...
    private func saveDownloadedModelsState() {
        ...
        print("💾 Saved downloaded models state: \(downloadedModels)")
    }
    ...
    private func getBundledModelPath(for model: WhisperKitModel) -> String? {
        ...
        for altName in alternativeNames {
            if let bundlePath = Bundle.main.path(forResource: altName, ofType: nil, inDirectory: "WhisperKitModels") {
                print("📦 Found bundled model with alternative name '\(altName)' for \(model.rawValue): \(bundlePath)")
                return bundlePath
            }
        }
        
        print("📦 Bundled model not found for: \(model.rawValue)")
        return nil
    }
    ...
    private func setupBundledModel(modelPath: String, modelName: String) async throws {
        ...
        if fileManager.fileExists(atPath: modelDir.path) {
            print("📦 Bundled model already exists in cache: \(modelDir.path)")
            return
        }
        ...
        if fileManager.fileExists(atPath: bundledURL.path) {
            ...
            print("📦 Bundled model copied to cache: \(modelDir.path)")
        } else {
            print("❌ Bundled model not found at: \(bundledURL.path)")
    ...
    private func initializeWhisperKit() async {
        print("🗣️ Initializing WhisperKit with bundled model priority...")
        ...
        do {
            ...
            if let modelPath = bundledModelPath {
                print("📦 Found bundled model at: \(modelPath)")
                print("🔧 Attempting to initialize WhisperKit with bundled model path: \(modelPath)")
                ...
                print("✅ WhisperKit initialized with bundled model: \(selectedModel.displayName)")
                
            } else {
                // フォールバック：従来のダウンロード方式
                print("📥 Bundled model not found, falling back to download method")
                print("🔧 Initializing WhisperKit with download: \(selectedModel.rawValue)")
                ...
                whisperKit = try await WhisperKit(config)
                print("✅ WhisperKit initialized with downloaded model: \(selectedModel.displayName)")
            }
    ...
    private func initializeWithSpecificModel(_ model: WhisperKitModel) async {
        print("🗣️ Initializing WhisperKit with specific model: \(model.rawValue)")
        ...
        do {
            ...
            print("📥 Starting WhisperKit initialization for \(model.displayName)...")
            whisperKit = try await WhisperKit(config)
            isInitialized = true
            initializationError = nil
            
            print("✅ WhisperKit initialized successfully with \(model.displayName)")
            
        } catch {
            print("❌ Failed to initialize WhisperKit with model \(model.rawValue): \(error)")
            initializationError = error
            isInitialized = false
    ...
    func transcribeAudioFile(at audioURL: URL) async throws {
        ...
        do {
            ...
            // 処理時間を推定
            let estimatedDuration = estimateProcessingTime(for: audioFileInfo.duration)
            await updateTranscriptionProgress(0.10, stage: "音声解析準備中...", estimatedRemaining: estimatedDuration)
            ...
            await updateTranscriptionProgress(0.15, stage: "文字起こし実行中...", estimatedRemaining: estimatedDuration * 0.85)
            print("🎯 Starting transcription: \(selectedModel.displayName)")
            print("🔧 Audio: \(String(format: "%.1f", audioFileInfo.duration))s, Model: \(selectedModel.rawValue)")
            print("🔧 WhisperKit State: initialized=\(isInitialized), instance=\(whisperKit != nil)")
            
            // 日本語専用設定で文字起こし（翻訳モード回避）- 過去の成功設定を復元
            let transcription = try await whisperKit.transcribe(
                audioPath: processedAudioURL.path,
                decodeOptions: DecodingOptions(
                    verbose: true,              // 詳細ログ有効（過去設定）
                    task: .transcribe,          // 翻訳ではなく転写
                    language: "ja",             // 明示的に日本語指定
                    temperature: 0.0,           // 決定論的設定で最高精度（過去設定）
                    temperatureIncrementOnFallback: 0.2,
                    temperatureFallbackCount: 5,
                    sampleLength: 224,
                    usePrefillPrompt: true,     // プロンプト使用
                    usePrefillCache: true,      // キャッシュ有効で一貫性向上（過去設定）
                    skipSpecialTokens: true,    // 特殊トークンをWhisperKitレベルで除去（過去設定）
            ...
            print("📝 Transcription completed: \(transcription.count) result(s)")
            
            let resultText: String
            if !transcription.isEmpty {
                let result = transcription.first!
                print("📝 Segments: \(result.segments.count), Text length: \(result.text.count)")
                
                // 最初の結果のテキストを使用
                let mainText = transcription.first?.text ?? ""
                print("🔍 RAW WhisperKit output: '\(mainText)'")
                
                // セグメント詳細もログ出力（クリーニング前）
                for (index, segment) in result.segments.enumerated() {
                    print("🔍 Segment \(index) RAW: '\(segment.text)' (start: \(segment.start), end: \(segment.end))")
                }
                
                // 音楽判定の後処理改善
                let cleanedText = postProcessTranscriptionResult(mainText)
                print("🔍 After postProcessing main text: '\(cleanedText)'")
                ...
                if cleanedText.isEmpty {
                    ...
                    let allSegmentTexts = transcription.flatMap { $0.segments }.compactMap { segment -> String? in
                        ...
                        print("🔍 Segment processing: '\(rawSegmentText)' -> '\(cleanedSegmentText)'")
            ...
            print("✅ Transcription completed in \(String(format: "%.2f", duration))s (\(finalResultText.count) chars)")
    ...
    private func preprocessAudioForWhisperKit(_ audioURL: URL) async throws -> URL {
        print("🎚️ Preprocessing audio for WhisperKit optimization...")
        ...
        print("🎚️ Audio analysis: peak=\(volumeAnalysis.peak), rms=\(volumeAnalysis.rms), dynamic_range=\(volumeAnalysis.dynamicRange)")
        ...
        print("🎚️ Audio preprocessing completed: \(processedURL.lastPathComponent)")
        return processedURL
    }
    ...
    private func optimizeAudioForWhisperKit(_ buffer: AVAudioPCMBuffer, analysis: (peak: Float, rms: Float, dynamicRange: Float)) -> AVAudioPCMBuffer {
        ...
        print("🎚️ WhisperKit optimization: rms_gain=\(rmsGain), peak_gain=\(peakGain), final_gain=\(finalGain)")
    ...
    private func removeSpecialTokens(from text: String) -> String {
        ...
        if text != cleanedText && !text.isEmpty {
            print("🔧 Token filter: '\(text)' -> '\(cleanedText)'")
        }
    ...
    private func retryWithSpeechFocusedSettings(audioURL: URL, whisperKit: WhisperKit) async throws -> String {
        ...
        if !retryTranscription.isEmpty {
            ...
            print("🔍 Retry RAW output: '\(retryText)'")
            ...
            if let retryResult = retryTranscription.first {
                for (index, segment) in retryResult.segments.enumerated() {
                    ...
                    let cleanedSegmentText = postProcessTranscriptionResult(rawSegmentText)
                    print("🔍 Retry Segment \(index) RAW: '\(rawSegmentText)' -> CLEANED: '\(cleanedSegmentText)' (start: \(segment.start), end: \(segment.end))")
                }
            }
    ...
    func changeModel(to model: WhisperKitModel) async {
        ...
        if selectedModel == model {
            print("⚠️ Model \(model.rawValue) is already selected")
            return
        }
        ...
        await MainActor.run {
            ...
            if !isAlreadyDownloaded {
                ...
                print("✅ Model \(model.displayName) already downloaded, initializing...")
            }
        }
        ...
        await MainActor.run {
            if self.isInitialized {
                ...
                print("✅ Model \(model.rawValue) successfully initialized")
            } else {
                self.downloadingModels.remove(model)
                self.downloadErrorModels.insert(model)
                self.downloadProgress[model] = 0.0
                print("❌ Model \(model.rawValue) initialization failed")
            }
        }
    }
    ...
    private func estimateDownloadProgress(for model: WhisperKitModel) async {
        print("📥 Starting model download estimation for \(model.displayName)")
        
        // 既にダウンロード済みかチェック
        if await isModelCached(model) {
            print("✅ Model \(model.displayName) already cached, skipping download")
            await MainActor.run {
                self.downloadProgress[model] = 1.0
                self.downloadedModels.insert(model)
                self.downloadingModels.remove(model)
        ...
        for step in 1...totalSteps {
            ...
            await MainActor.run {
                ...
                print("📊 Download progress for \(model.displayName): \(Int(adjustedProgress * 100))%")
            }
            
            try? await Task.sleep(nanoseconds: UInt64(stepDuration * 1_000_000_000))
        }
        
        print("📥 Download estimation completed for \(model.displayName)")
    }
    ...
    private func isModelCached(_ model: WhisperKitModel) async -> Bool {
        ...
        if exists && isDirectory.boolValue {
            ...
            do {
                ...
                if hasMLFiles {
                    print("✅ Model \(model.displayName) found in cache at \(modelPath.path)")
                    return true
                }
            } catch {
                print("⚠️ Error checking model cache: \(error)")
            }
        }
        
        print("📥 Model \(model.displayName) not found in cache, download required")
        return false
    }
    ...
    func extractSegmentData(from transcription: [TranscriptionResult]) -> [TranscriptionSegment] {
        ...
        let segments = result.segments.compactMap { segment -> TranscriptionSegment? in
            ...
            guard !cleanedText.isEmpty else {
                print("🔍 Segment filtered out: '\(rawText)' -> empty")
                return nil
            }
        ...
        print("📊 Extracted \(segments.count) segments with timestamps (after filtering)")
        return segments
    }
    ...
    /// 文字起こし進捗の更新
    @MainActor
    private func updateTranscriptionProgress(_ progress: Float, stage: String, estimatedRemaining: TimeInterval? = nil) {
        self.transcriptionProgress = max(0.0, min(1.0, progress))
        self.transcriptionStage = stage
        self.estimatedTimeRemaining = estimatedRemaining
        
        print("📊 Progress: \(Int(progress * 100))% - \(stage)")
        if let remaining = estimatedRemaining {
            print("⏱️ Estimated remaining: \(Int(remaining))s")
        }
    }
    
    /// 音声長から処理時間を推定
    private func estimateProcessingTime(for duration: TimeInterval) -> TimeInterval {
        // 実測データベース: 音声長 vs 処理時間
        // 60秒 → 2.8秒 (Real Time Factor: 0.046)
        let realTimeFactor: Double = 0.05 // 安全マージン含む
        return duration * realTimeFactor
    }
